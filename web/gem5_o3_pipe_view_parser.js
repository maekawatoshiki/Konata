...
class Gem5O3PipeViewParser {
    constructor() {
        ...
        // Lines processed before yielding control to the browser
        this.yieldInterval_ = 4096;
    ...
    startParsing() {
        ...
        const smallFile = size > 0 && size <= 25 * 1024 * 1024;
    ...
    async parseLineBody_(line) {
        ...
        if (args[0] != "O3PipeView") {
            ...
            if (
                this.parsingExLogLastGID_ == -1 ||
                sn <= this.opListBody_.parsedLastID
            ) {
                // 既にドレインされたものは諦める
                // Drop drained ops
            } else {
        ...
        if (this.updateTimer_ < 0) {
            ...
            let percent = 0;
            if (this.file_ && this.file_.fileSize) {
                percent = (1.0 * this.file_.bytesRead) / this.file_.fileSize;
            }
            this.updateCallback_(Math.min(percent, 0.99), this.updateCount_);
            this.updateCount_++;

            if (this.isGem5O3PipeView) {
                await this.drainParsingOps_(false);
            }
        }
    ...
    async detectTicksPerClock(force) {
        ...
        let sortedTicks = rawTicks.sort((a, b) => {
            return a - b;
        });
    ...
    /** @param {boolean} force */
    async drainParsingOps_(force) {
        await this.detectTicksPerClock(force);
        if (this.ticks_per_clock_ == -1) {
            return;
        }
        ...
        let seqNumList = Object.keys(this.parsingOpList_).sort((a, b) => {
        ...
        for (let seqNumStr of seqNumList) {
            ...
            // Update clock cycles
            op.fetchedCycle =
                op.fetchedCycle / this.ticks_per_clock_ - this.cycle_begin_;
            op.retiredCycle =
                op.retiredCycle / this.ticks_per_clock_ - this.cycle_begin_;
            ...
            if (op.prodCycle != -1)
                op.prodCycle =
                    op.prodCycle / this.ticks_per_clock_ - this.cycle_begin_;
            if (op.consCycle != -1)
                op.consCycle =
                    op.consCycle / this.ticks_per_clock_ - this.cycle_begin_;
            ...
            if (this.lastGID_ > seqNum) {
                console.log(
                    `Miss parsed op: seqNum: ${seqNum} lastGID: ${this.lastGID_}. BUFFERED_SIZE must be bigger.`,
                );
            }
        ...
        // ある程度バッファしておく
        for (
            let i = this.opListBody_.parsedLastID + 1;
            i <= this.secondParsingOpList_.parsingLastID;
            i++
        ) {
            ...
            if (!op.flush) {
                ...
                // RID テーブルを更新して hideFlushedOps で RID ベースのスクロールが働くようにする
                ...
                // Dummy RID
                op.rid =
                    this.opListBody_.parsedLastRID +
                    this.lastID -
                    this.lastNotFlushedID;
            }
                ...
                for (let s of exLog.srcs) {
                    ...
                    if (s in this.depTable_) {
                        ...
                        if (prod.prodCycle < op.consCycle) {
                            op.prods.push(
                                new Dependency(prod.id, type, op.prodCycle),
                            );
                            prod.cons.push(
                                new Dependency(op.id, type, op.consCycle),
                            );
                        }
                    }
                }
    ...
    async finishParsing() {
        ...
        // 未処理の命令を強制処理
        await this.drainParsingOps_(true);
        ...
        this.opListBody_.setParsedLastID(
            this.secondParsingOpList_.parsingLastID,
        );
            ...
            if (args[1] == " user") {
                ...
                for (let i = 2; i < args.length; i++) {
                    ...
                    if (args[i].match(/ (Renaming)|(Looking)/)) {
                        ...
                        let dst = args[i].match(
                            /\(([a-zA-Z]+)\) to physical reg (\d+) \(\d+\)/,
                        );
                        ...
                        let src = args[i].match(
                            /got phys reg (\d+) \(([a-zA-Z]+)\)/,
                        );
