...
class Gem5O3PipeViewParser {
	 ...
	 */
	setFile(file, updateCallback, finishCallback, errorCallback, config = null) {
		this.file_ = file;
	...
	async drainParsingOps_(force) {
		...
		for (let seqNumStr of seqNumList) {
			...
			for (let laneID in op.lanes) {
				...
				for (let stage of lane.stages) {
					stage.startCycle =
						stage.startCycle / this.ticks_per_clock_ - this.cycle_begin_;
					stage.endCycle =
						stage.endCycle / this.ticks_per_clock_ - this.cycle_begin_;
		...
		for (
			...
			if (!op.flush) {
				...
				// Dummy RID
				op.rid =
					this.opListBody_.parsedLastRID + this.lastID - this.lastNotFlushedID;
			}
	...
	parseExLog(op, parseCycleRange) {
		...
		while (logList.length) {
			...
			if (op.lastParsedStage.labels != "") op.lastParsedStage.labels += "\n";
